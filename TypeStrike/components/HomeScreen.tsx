...
@keyframes ts-card-in { from{opacity:0;transform:translateY(24px) scale(.96)} to{opacity:1;transform:none} }
...
@keyframes title-glow { 0%,100%{filter:drop-shadow(0 0 22px rgba(0,255,136,.6))} 50%{filter:drop-shadow(0 0 34px rgba(0,255,136,.9))} }
@keyframes join-pulse { 0%,100%{box-shadow:0 0 40px rgba(0,255,136,.5),0 0 80px rgba(0,255,136,.2)} 50%{box-shadow:0 0 55px rgba(0,255,136,.7),0 0 100px rgba(0,255,136,.3)} }
...
.ts-card-wrap {
  ...
  background: conic-gradient(from var(--border-angle),#00ff88,#22d3ee,#8844ff,#ff0080,#ffd700,#00ff88);
...
.ts-join-btn:hover { filter:brightness(1.18) saturate(1.15); transform:translateY(-2px) scale(1.02); }
.ts-join-btn:disabled { background:rgba(255,255,255,.06); animation:none; box-shadow:none; }
.ts-title-text {
  background: linear-gradient(135deg,#00ff88,#22d3ee,#ffd700,#ff0080,#00ff88);
  background-size: 250% 100%;
...
const RULES = [
  { key: 'TYPE',      col: '#00ff88', desc: 'Type the passage exactly as shown — character by character' },
  { key: 'BACKSPACE', col: '#ffd700', desc: 'Delete mistakes before continuing — no skipping errors' },
  { key: 'FINISH',    col: '#22d3ee', desc: 'First to complete the full passage wins the lobby' },
];
...
export default function HomeScreen({ onQueue, onSolo, onBotTrial, trialComplete, playerName: savedName, playerColor: savedColor, balance = 0, isLoggedIn, soloRunsToday, isDev, points, winStreak = 0, lossStreak = 0 }: Props) {
  ...
  useEffect(() => {
    if (document.getElementById('ts-home-css')) return;
    const el = document.createElement('style'); el.id = 'ts-home-css'; el.textContent = CSS;
    document.head.appendChild(el);
  }, []);

  function resolve() { return (savedName ?? '').trim() || `TYPER-${Math.floor(Math.random() * 9000) + 1000}`; }
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#08080f', position: 'relative', overflow: 'hidden', padding: '12px 16px', boxSizing: 'border-box' }}>

      {/* Background blobs */}
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,255,136,.14) 0%, transparent 65%)', top: -200, left: -150, animation: 'blob-a 9s ease-in-out infinite, hue-spin 12s linear infinite' }} />
        <div style={{ position: 'absolute', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,180,255,.12) 0%, transparent 65%)', top: -150, right: -100, animation: 'blob-b 11s ease-in-out infinite, hue-spin 9s linear infinite reverse' }} />
        <div style={{ position: 'absolute', width: 450, height: 450, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,0,128,.10) 0%, transparent 65%)', bottom: -120, left: -80, animation: 'blob-c 10s ease-in-out infinite, hue-spin 15s linear infinite' }} />
        <div style={{ position: 'absolute', width: 350, height: 350, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,200,0,.07) 0%, transparent 65%)', top: '40%', left: '55%', animation: 'blob-a 13s ease-in-out infinite reverse, hue-spin 10s linear infinite' }} />
      </div>
      ...
      {/* Speed lines */}
      {(['rgba(0,255,136', 'rgba(34,211,238', 'rgba(255,0,128', 'rgba(255,200,0', 'rgba(140,0,255'] as const).map((c, i) => (
      ...
      {/* Back link */}
      <a href="/" target="_top" style={{ position: 'absolute', top: 56, left: 20, zIndex: 2, fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,.3)', textDecoration: 'none', fontFamily: 'inherit', transition: 'color .15s' }}
      ...
      {/* Card */}
      <div className="ts-card-wrap" style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 468, flexShrink: 0, animation: 'ts-card-in 0.5s cubic-bezier(.22,1,.36,1) both' }}>
        <div style={{ background: 'rgba(4,4,14,.92)', borderRadius: '20.5px', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12, boxShadow: '0 24px 80px rgba(0,0,0,.8)' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 4, background: 'linear-gradient(90deg,#00ff88,#22d3ee,#ffd700,#ff0080,#00ff88)', backgroundSize: '200% 100%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animation: 'rainbow-shift 3s linear infinite' }}>Paigon · Skill Gaming</div>
            <h1 className="ts-title-text" style={{ fontSize: '2.8rem', fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.05, marginBottom: 4 }}>
              TYPESTRIKE
            </h1>
            <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,.38)', letterSpacing: '0.04em' }}>
              Type Fast · Finish First
            </p>
          </div>
          ...
          <div style={{ borderRadius: 12, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.07)', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ fontSize: '0.52rem', fontWeight: 800, letterSpacing: '0.24em', color: 'rgba(255,255,255,.22)', textTransform: 'uppercase', marginBottom: 2 }}>How It Works</div>
            {RULES.map(({ key, col, desc }) => (
              <div key={key} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <div style={{ minWidth: 58, textAlign: 'center', fontSize: '0.55rem', fontWeight: 800, color: col, letterSpacing: '0.06em', background: `${col}18`, padding: '2px 6px', borderRadius: 4, marginTop: 1, boxShadow: `0 0 8px ${col}44`, border: `1px solid ${col}44` }}>{key}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <button
              className={isPaid && !trialLocked && !canAfford ? '' : 'ts-join-btn'}
              onClick={handleQueue}
          ...
          <p style={{ textAlign: 'center', fontSize: '0.7rem', color: 'rgba(255,255,255,.18)', lineHeight: 1.6 }}>
            {trialComplete ? 'Skill-based · Same passage · No house edge' : 'Complete trial to unlock competitive lobbies · 18+ only'}
          </p>
