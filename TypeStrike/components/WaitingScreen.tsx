...
export default function WaitingScreen({ onLeave, queueCount }: Props) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 28, background: '#08080f', position: 'relative', overflow: 'hidden' }}>
      <style>{`
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        ...
        <div style={{ position: 'absolute', width: 300, height: 300, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,255,136,.09) 0%, transparent 70%)', top: '45%', left: '60%', animation: 'blob-wait-a 11s ease-in-out infinite reverse, ring-hue 8s linear infinite' }} />
      </div>
      ...
      <div style={{ position: 'relative', width: 100, height: 100 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#00ff88', boxShadow: '0 0 12px #00ff88, 0 0 24px rgba(0,255,136,.4)', animation: 'ring-hue 3s linear infinite' }} />
        </div>
      </div>

      <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <div style={{ fontSize: '1.15rem', fontWeight: 800, marginBottom: 8, background: 'linear-gradient(135deg,#00ff88,#22d3ee,#ff0080,#ffd700,#00ff88)', backgroundSize: '250% 100%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animation: 'rainbow-shift 2.5s linear infinite' }}>
          {hasCount && joined >= min ? 'Lobby Filling…' : 'Finding Typers'}
        </div>
