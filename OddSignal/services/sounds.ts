...
export const Sounds = {
  /** Correct shape — pitch rises with the run so progress is audible. */
  correct: (run = 0) => tone(540 + Math.min(run, 12) * 38, 0.12, 'triangle', 0.22),
  /** Wrong shape; the lockout that follows is the real cost. */
  wrong: () => { tone(200, 0.18, 'sawtooth', 0.17); noise(0.1, 0.08, 400); },
  /** Lockout expired, cards live again. */
  unlock: () => tone(480, 0.09, 'sine', 0.15),
