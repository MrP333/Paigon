...
export const Sounds = {
  /** Target struck — pitch rises with the streak so a run sounds like one. */
  hit: (streak = 0) => tone(520 + Math.min(streak, 10) * 45, 0.11, 'triangle', 0.22),
  /** Clicked nothing. */
  miss: () => noise(0.13, 0.12, 500),
  /** Clicked a decoy, or a real target timed out — the chain is broken. */
  chainBreak: () => { tone(220, 0.16, 'sawtooth', 0.16); tone(150, 0.22, 'sine', 0.13, 0.04); },
  /** Multiplier stepped up. */
  streakUp: () => [0, 0.06, 0.12].forEach((d, i) => tone(600 + i * 190, 0.13, 'sine', 0.17, d)),
